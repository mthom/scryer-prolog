...
|--------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|-----------------------------------|-------------------|
| `scryer_machine_new()`               | Initializes a new Prolog machine. It is the responsibility of the caller to call `scryer_machine_free()`.                   | None                              | `*mut Machine`    |
| `scryer_machine_free()`              | Deallocates memory for the Prolog machine created with `scryer_machine_new()`.                                              | `*mut Machine`                    | None              |
| `scryer_start_new_query_generator()` | Initializes a new query generator with provided Prolog source code (C string). Returns JSON status.                         | `(*mut Machine, *const c_char)`   | `*mut QueryState` |
| `scryer_cleanup_query_generator()`   | Cleans up resources associated with the query generator. Returns JSON status.                                               | `(*mut Machine, *mut QueryState)` | `*mut c_char`     |
| `scryer_run_query_generator()`       | Runs the query generator, returning results as JSON. Expects initialization from previous calls.                            | `(*mut Machine, *mut QueryState)` | `*mut c_char`     |
| `scryer_load_module_string()`        | Loads Prolog source code into the "facts" module. Returns JSON status.                                                      | `(*mut Machine, *const c_char)`   | `*mut c_char`     |
| `scryer_consult_module_string()`     | Consults (loads and runs) facts from a C string. Returns JSON status.                                                       | `(*mut Machine, *const c_char)`   | `*mut c_char`     |
| `scryer_run_query()`                 | Executes a single Prolog query with provided C-style string input. Returns JSON results.                                    | `(*mut Machine, *const c_char)`   | `*mut c_char`     |
| `scryer_free_c_string()`             | Frees memory allocated for C strings returned by other functions. **IMPORTANT:** Always call after using the result string! | `*mut c_char`                     | None              |
