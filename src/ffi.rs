...
impl FunctionImpl {
    ...
    unsafe fn call_cstr(&self, args: &[Arg], _: &mut Arena) -> Result<Value, FfiError> {
        let ptr = unsafe { self.cif.call::<*mut c_void>(self.code_ptr, &args) };
        Ok(Value::CString(
            unsafe { CStr::from_ptr(ptr.cast()) }.to_owned(),
        ))
    }
...
impl<'val> Deref for PointerArgs<'_, '_> {
    type Target = [Arg];
...
impl FfiType {
    fn from_atom(atom: &Atom) -> Self {
        match atom {
            ...
            atom!("f32") => Self::F32,
            atom!("f64") => Self::F64,
            struct_name => Self::Struct(struct_name.clone()),
        }
    }
...
impl<'val> ArgValue<'val> {
    fn new(
        ...
        match arg_type {
            ...
            FfiType::Void => return Err(FfiError::InvalidArgumentType),
        }
    }
