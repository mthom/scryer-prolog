...
impl SystemClauseType {
    pub(crate) fn name(&self) -> ClauseName {
        match self {
            ...
            &SystemClauseType::SetReadTimeout => clause_name!("$set_read_timeout"),
            &SystemClauseType::SetWriteTimeout => clause_name!("$set_write_timeout"),
    ...
    pub(crate) fn from(name: &str, arity: usize) -> Option<SystemClauseType> {
        match (name, arity) {
            ...
            ("$set_read_timeout", 2) => Some(SystemClauseType::SetReadTimeout),
            ("$set_write_timeout", 2) => Some(SystemClauseType::SetWriteTimeout),
